...
function Conversation({ onClose }: { onClose: () => void }) {
  ...
  return <section className="agent-panel" aria-label="AutomateMeJay AI assistant">
    <header>
      ...
      <button type="button" aria-label="Close assistant" onClick={onClose}><X size={18} /></button>
    </header>
...
export function JasonAgentConversation({ agentId, onClose }: { agentId: string; onClose: () => void }) {
  return <ConversationProvider agentId={agentId}><Conversation onClose={onClose} /></ConversationProvider>;
}
