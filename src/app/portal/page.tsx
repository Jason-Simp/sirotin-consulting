...
export default async function PortalPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!membership) {
    ...
    return <main className="portal-shell portal-shell-single"><section className="portal-empty portal-awaiting"><ShieldCheck size={34} /><p className="section-label">/ Private client workspace</p><h1>Your account is secure. No workspace has been activated yet.</h1><p>After you begin a risk-free first week or monthly engagement, Jason activates your organization here. This is where messages, files, testing, approvals, signed work, and billing live.</p><div className="button-row"><Link className="button button-primary" href="/start">Start risk-free</Link><Link className="button button-secondary" href="/book">Book a consultation</Link></div><form action={signOut}><button className="portal-text-button" type="submit"><LogOut size={15} /> Sign out</button></form></section></main>;
  }
