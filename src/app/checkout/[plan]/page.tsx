...
const planData = {
  ...
  weekly: { name: "Weekly Partner", price: "$350", cadence: "each week", notice: "Cancel anytime through your membership" },
...
export default async function CheckoutPage({ params }: { params: Promise<{ plan: string }> }) {
  ...
  return <main className="subpage"><SubpageHeader /><div className="checkout-layout"><section><p className="section-label">/ {selected.name}</p>{testMode && <span className="test-badge">Stripe sandbox · No real charge</span>}<h1>{selected.price}<span>{selected.cadence}</span></h1><p>{selectedPlan === "first-week" ? "A paid seven-day introduction with a straightforward full money-back guarantee. Continue only if you choose to." : "An ongoing AI automation relationship with clear capacity, simple renewal, and easy cancellation."}</p><div className="checkout-summary"><span><Check size={15} /> One primary directing stakeholder</span><span><Check size={15} /> One active workstream</span><span><Check size={15} /> {selected.notice}</span>{selectedPlan === "first-week" ? <span><Check size={15} /> No automatic conversion</span> : <span><Check size={15} /> Access through the current paid period</span>}</div></section><aside><CheckoutForm plan={selectedPlan} /></aside></div></main>;
}
