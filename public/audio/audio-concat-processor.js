      ...
      switch (data.type) {
        case "setFormat":
          ...
          if (globalThis.LibSampleRate && sampleRate !== data.sampleRate) {
            globalThis.LibSampleRate.create(1, data.sampleRate, sampleRate).then((resampler) => {
  ...
  process(_, outputs) {
    ...
    for (let index = 0; index < output.length; index++) {
      if (!this.currentBuffer) {
        ...
        if (this.resampler) this.currentBuffer = this.resampler.full(this.currentBuffer);
        this.cursor = 0;
      }
